...
var forumLink = httpsforum.tribalwars.netindex.phpthreadshide-barbarians-in-la.286468;
...
function main(){
    ...
    if(window.location.href.indexOf(`${runScreen}`)==-1)
    {
        ...
        window.location.href = window.location.pathname+ `${sittersitter+&}${runScreen}`;
        return;
    }
...
function hitCountApi(){
    $.getJSON(`httpsapi.countapi.xyzhitfmthemasterScripts${countapikey}`, function(response) {
...
function setHTML(){
            ...
            div id=${scriptTag}_popup_content class=fm_popup_content
                        ...
                        tbody class=vis
                            tr class= row_b
                                ...
                                td class=${scriptTag}_td_farm_icon
                                    a href=javascriptvoid(0); class=fm_centered ${scriptTag}_farm_keybind ${scriptTag}_farm_icon farm_icon farm_icon_a data-farmtype=aa
                                td
                                td class=${scriptTag}_td_farm_icon
                                    a href=javascriptvoid(0); class=${scriptTag}_farm_keybind ${scriptTag}_farm_icon farm_icon farm_icon_b data-farmtype=ba
                                td
                            tr
                            tr class= row_b
                                th
                ...
                div id=${scriptTag}_LAlist
                    table
                        thead
                            tr
                                th style=min-width70px;Villageth
                                th style=min-width40px;img src=graphicrechts.pngth
                                th colspan=5LAth
                            tr
                        thead
    ...
    style
        ...
        .fm_popup_container {
            ...
            -moz-border-image url(graphicpopupborder.png) 9 19 19 19 repeat;
            -webkit-border-image url(graphicpopupborder.png) 9 19 19 19 repeat;
            -o-border-image url(graphicpopupborder.png) 19 19 19 19 repeat;
            border-image url(graphicpopupborder.png) 19 19 19 19 repeat;
            display block;
            position fixed;
            top 8%;
            left 70%;
        ...
        .fm_popup_content {
            ...
            height100%;
            overflow hidden;
            background-image url('graphicpopupcontent_background.png');
        }
        .fm_centered {
            text-align center;
        }
...
function addLARow(village){
    ...
    tr class=${addLARow.counter%2row_arow_b}
        tda href=${window.location.pathname}${sittersitter+&}&screen=info_village&id=${village.id} target=_blank${parseInt(village.xy1000)}${village.xy%1000}atd
        td${village.distance}td
        td class=${scriptTag}_td_farm_icona href=javascriptvoid(0); class=fm_centered ${scriptTag}_farm_icon ${scriptTag}_sendFarm farm_icon farm_icon_a data-farmtype=a data-villagexy=${village.xy}atd
        td class=${scriptTag}_td_farm_icona href=javascriptvoid(0); class=${scriptTag}_farm_icon ${scriptTag}_sendFarm farm_icon farm_icon_b data-farmtype=b data-villagexy=${village.xy}atd
    tr`);
}
...
function makeLATable(){
    ...
    barbs.sort(function(a, b){
        ...
        let [xa,ya] = [Math.floor(a.xy1000), a.xy%1000];
        let [xb,yb] = [Math.floor(b.xy1000), b.xy%1000];
...
function addAuthor(cointainerSelector){
    let authorHTML = `
    div class=fmAuthor    
        bScriptb a href=${forumLink}${scriptName}a
        br
        bAuthorb a href=httpsforum.tribalwars.netindex.phpmembersthe-quacks.124200fmthemaster aka The Quacksa
    div
    style
        .fmAuthor{
...
function startLoader(length)
{
    ...
    div id=progressbar class=progress-bar
        span class=count label0${length}span
        div id=progressspan class=count label style=width ${width}px;0${length}spandiv
    div`);
}
...
async function getFirstFarmPage(){
    ...
    loadingLAstuff = true;
    $.get(`game.php${sittersitter+&}village=${game_data.village.id}&screen=am_farm&Farm_page=0`, async (data)= {
            const parser = new DOMParser();
            const doc= await parser.parseFromString(data, texthtml);
            ...
            let toggleBox =(key, url, val)={
                ...
                TribalWars.post(url,null,{extended0+true, target_screenam_farm, [key]val});
            };
            ...
            executeQueue(postGetQueue, 300, {loadTexttoggling LA options, callback()=getBarbsInLA(0)});
...
async function getBarbsInLA(page, depth=0, npages=undefined) {
    ...
    let url = `game.php${sittersitter+&}village=${game_data.village.id}&screen=am_farm&Farm_page=${page}`;
    $.get(url, async (data)= {
        console.log(success);
        const parser = new DOMParser();
        const doc= await parser.parseFromString(data, texthtml);
        const pageSelector = $(.paged-nav-itemlast, doc); 
        ...
        if(!npages){
            ...
            for(var i = 1; i  npagesLA; i++){
                const j =i;
                pageQueue.push(()=getBarbsInLA(j,0,npagesLA)); jshint ignoreline
            }
            executeQueue(pageQueue, 300, {loadTextloading LA pages,callback()={
                makeLATable();
                executeQueue(toToggleBack, 300, {loadTexttoggling LA options back, callback ()={toToggleBack = [];loadingLAstuff=false;}});
            }});
...
function farmVillage (xy, type) {
    ...
    let mpFarm = type==amp_farm_amp_farm_b;
...
function getCache(){
    ...
    fmMapLASettings = cachedSettings  JSON.parse(cachedSettings)  {ignoreLAfalse, replaceFilterstrue};
}
...
function getHTMLOptions(){
    ...
    $(`#${scriptTag}_ignoreLA`).each(function(){
        ...
        let display = isCheckednoneblock;
